...
(defun memset (obj name val count)
	(cond
		((= count 0) T)
		(T
			(put obj name
				(cons val (get obj name))
			)(memset obj name val (- count 1)))
	)
)
(defun getNth (lst idx)
	(cond
		((null lst) Nil)
		((= idx 0) (car lst))
		(T (getNth (cdr lst) (- idx 1)))
	)
)
(defun setNth (lst idx val)
	(cond
		((null lst) Nil)
		((= idx 0) (cons val (cdr lst)))
		(T (cons (car lst) (setNth (cdr lst) (- idx 1) val)))
	)
)
...
(defun fillColor (edgelist colorlist)
	(cond
		((null edgelist) colorlist)
		(T (let ((elem (first (first edgelist))))
			(cond
				((hasinlist colorlist elem)
					(fillColor (cdr edgelist) colorlist))
				(T (fillColor (cdr edgelist) (cons (list elem 0) colorlist)))
			)
			)
		)
	)
...
(defun loadGraph ()
	...
	(put 'G 'color (fillColor (get 'G 'edge) Nil))
	(put 'G 'error Nil)
	(put 'G 'answer Nil)
)
...
(defun porcess ()
	(loadGraph)
	(getforest 1)
	; (print 'edge)(print (get 'G 'edge))
	; (print 'color)(print (get 'G 'color))
	(print 'error)(print (get 'G 'error))
	(print 'answer)(print (get 'G 'answer))
)